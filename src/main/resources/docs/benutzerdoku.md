...
Diese Aufgaben laufen nur auf `macOS`. `packageMacOsInstallers` erzeugt DMG und PKG gemeinsam. `macosInstall` baut das App-Bundle und ersetzt ausschließlich eine vorhandene CADas-App mit passender Bundle-ID atomar unter `/Applications/CADas.app`. Symbolische Ziele und fremde App-Bundles werden abgelehnt.
...
* Schalter für `Raster`, `Raster-Snap`, `Objekte` und `Gelände 2D`
...
Über **Gelände** in der Werkzeugleiste beziehungsweise `Werkzeuge` > `Geländehöhen bearbeiten` erfasst du für jede äußere Ecke des Gebäudes die Geländehöhe in Zentimetern relativ zum Boden der untersten Etage. CADas leitet die äußeren Ecken automatisch aus der Außenwand-Schleife des Grundrisses ab, unterstützt dabei auch konkave Gebäudeformen wie L-Grundrisse und erhält vorhandene Höhen beim erneuten Öffnen des Dialogs. Im Dialog legst du zusätzlich die `Darstellungsbreite` des Geländebands fest.

In der 2D-Ansicht zeigt `Gelände 2D` das Gelände nur als Band außerhalb des Gebäudes. Die 3D-Ansicht zeigt dasselbe Gelände hellbraun um das Gebäude herum. In den orthogonalen Seitenansichten erscheint die Geländeaußenkante, sodass Gefälle und Steigungen kontrollierbar bleiben.
...
Das Raster liegt oberhalb der Modellinhalte, aber unterhalb der aktuellen Auswahl und ihrer Bearbeitungsgriffe. Über `Raster` in der Werkzeugleiste oder `Optionen` > `Raster anzeigen` blendest du es ein oder aus. Beim starken Herauszoomen zeigt CADas automatisch ein gröberes Vielfaches der eingestellten Rasterweite.
...
* Das Raster kann sichtbar bleiben oder separat ausgeblendet werden, wirkt aber nicht mehr als Fanghilfe.
