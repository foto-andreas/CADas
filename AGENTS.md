...
* NIEMALS LINKS AUF VERZEICHNISSE AUSSERHALB DES WORKSPACE SETZEN!!!
...
* Die Dachschrägen-Schnellfunktion startet an der Innenkante einer ausgewählten Wand, übernimmt Sockelhöhe und horizontale Breite, senkt die gewählte Wand ab, teilt die beiden angrenzenden Seitenwände an der oberen Schrägenkante und aktualisiert die Decke des erkannten Raums. Ein Raum kann an mehreren Seiten unabhängige Dachschrägen besitzen; ihre dauerhaften Bodenmarkierungen bleiben bei erneuter Raumableitung und DXF-Persistenz erhalten.
* Dachfenster sind eigenständige, raum- und dachschrägengebundene Fachobjekte. Sie werden in 2D auf der gewählten Schräge platziert, in Gebäude- und Etagen-DXF gespeichert und in 3D mit Neigung der zugehörigen Dachschräge gerendert.
...
* Dreidimensionale IFC2x3-Objekte werden mindestens aus `IFCFACETEDBREP`- und `IFCEXTRUDEDAREASOLID`-Körpern importiert. `IFCCOLOURRGB`-Oberflächenfarben werden über die Stilzuordnung bis ins 3D-Rendering erhalten. DXF-TrueColor und grundlegende ACI-Farben werden ebenfalls übernommen.
* RFA-Objekte werden plattformneutral über eine gleichnamige IFC- oder 3D-DXF-Begleitgeometrie importiert. Beim Registrieren werden RFA und Begleitdatei gemeinsam übernommen; IFC wird gegenüber DXF bevorzugt. Die proprietäre RFA-Datei wird niemals als vermeintlich offenes Format direkt interpretiert.
...
* Boden- und Deckenflächen nutzen dasselbe wassergeführte Flächenheizungsmodell. Pro Raum und Fläche werden Vorlauf, Rücklauf, Verlegeabstand, daraus abgeleiteter Kurvenradius, Rohrdurchmesser, maximale Rohrlänge und Wandabstand gespeichert. CADas schlägt getrennte Heizkreise mit Meander- oder Schneckenverlegung vor, teilt zu lange Kreise automatisch und erhält frei bearbeitbare polygonale Bereiche einschließlich L-Formen. PDF-Bauzeichnungen führen je beheizter Etage getrennte Heizplanseiten für Fußboden und Decke.
...
* Das Raum-Kontextmenü benennt ausschließlich den angeklickten Raum über dessen stabile ID um. Ein Kontextklick macht das Ziel zur alleinigen Auswahl; Raummaße und weitere Eigenschaften bleiben beim Umbenennen unverändert.
...
* `./gradlew packageMacOsInstallers` erzeugt DMG und PKG im Workspace. `./gradlew macosInstall` darf ausschließlich das gebaute `CADas.app` atomar nach `/Applications/CADas.app` kopieren. Es muss Quellpfad, Bundle-ID und exaktes Ziel validieren, symbolische Ziele ablehnen und darf keine externen Verzeichnislinks erzeugen.
