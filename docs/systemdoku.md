...
* `SurfaceMaterialListService` erzeugt aus den sichtbaren Belägen eine Materialliste mit Fläche, Stückzahl, Materialfläche, Schnitten und Raum-/Flächen-Komplexität. Boden- und Deckenflächen werden über die orthogonale Raumzerlegung bewertet; Wandbeläge nutzen die vorhandenen maximalen Wandrechtecke mit ausgesparten Türen, Fenstern und anstoßenden Innenwänden. Polygonale Wandoberkanten werden für Berichte zusätzlich in Sockel- und Schräge-Anteile getrennt. Raumtabellen werden nach Etagen gegliedert; beheizte Raumobjekte werden mit ihrer Heizart getrennt nach `FBH`, `DH`, `Flächenheizung` und `Heizelement` summiert.
